...
// hand. The artwork carries the name, so callers no longer render the word themselves.
//
// TWO images rather than one recoloured asset: the lock-up is two-coloured (brand-ink text,
// brand-red house), so the CSS-mask trick ContactIcon uses cannot work — a mask paints
// everything in a single currentColor and the house would stop being red. The ink half is
// near-black and would vanish on the dark theme, hence the dark variant. Tailwind's dark:
// variant is wired to [data-theme="dark"] (tailwind.config.js), so the swap is pure CSS: no
// JS, no theme hook, and no flash of the wrong one before hydration.
//
// Both files are generated by scripts/gen-icons.mjs from brandImages/logo_new_2.png — do not
// point this at the source artwork, which is frozen at today's hexes instead of following
// brand.config.json.
//
// Sized by HEIGHT only (e.g. "h-8"); the width follows the 2.4:1 aspect ratio.
//
// The pair is wrapped in ONE span rather than returned as a fragment. Every caller drops this
// into a flex container, and two bare children would both count as flex items: the sidebar's
// justify-between would space four children instead of three, and align-items:stretch would
// blow each <img> out to the container's full width — object-contain then centres the artwork
// inside that, so a left-aligned lock-up silently renders centred. One inline-flex child with
// the height on it, and the images filling it, behaves like the single <img> it replaced.
export function Wordmark({ className }: { className?: string }) {
  return (
    <span className={cn("inline-flex shrink-0 items-center", className ?? "h-8")}>
